  ...
  return (
    <div className=' absolute bg-gradient-to-b from-black w-full z-10 flex justify-between'>
      ...
      {user && <div className=' p-8 flex flex-col items-center cursor-pointer gap-2'>  
                                                                  {<div className=' flex'> <p className=' text-lg font-semibold'>{`Hye , ${user.displayName}`}
                                                                                                                                                                                                                        </p> <img src={user.photoURL}alt='user-logo' className=' w-8' onClick={() => setShowSignOut(!showSignOut)} /></div>}
                                                                     {showSignOut && <button className=' text-white font-semibold text-lg' onClick={SignOutHandler}> Sign Out</button>}</div>}
    </div>
  )
}
